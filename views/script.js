...
async function loadData() {
  try {
    ...
    books = booksData
      .filter(book => book.dates_read && book.dates_read.length > 0 && book.dates_read[0] !== 'not set')
      .map(book => {
        // Get the most recent read date
        const mostRecentDate = book.dates_read[book.dates_read.length - 1];
        return {
          book_id: book.book_id,
          title: book.raw_title.split('\n')[0].trim(),
          author: book.author.split(',').reverse().join(' ').trim(),
    ...
    console.log(`Loaded ${books.length} books and ${allQuotes.length} quotes`);
...
function selectDailyQuote() {
  ...
  const availableQuotes = allQuotes.filter(q => !usedQuoteIds.has(q.quote_id));
...
document.getElementById('guessForm').addEventListener('submit', function(e) {
  ...
  const isValidBook = books.some(book =>
    book.title.toLowerCase() === title && book.author.toLowerCase() === author
  );
  ...
  if (title === correctTitle && author === correctAuthor) {
    feedbackBox.textContent = `🎉 Correct! You got it in ${attemptCount} attempt${attemptCount === 1 ? '' : 's'}!`;
    feedbackBox.className = "feedback success";
    disableInputs();
  } else if (attemptCount >= MAX_ATTEMPTS) {
    feedbackBox.textContent = `❌ Failed! The answer was "${currentQuote.book.title}" by ${currentQuote.book.author}`;
    feedbackBox.className = "feedback error";
    disableInputs();
  } else if (author === correctAuthor && title !== correctTitle) {
    feedbackBox.textContent = `✍️ You got the author! Now guess the title. (${attemptCount}/${MAX_ATTEMPTS})`;
    feedbackBox.className = "feedback partial";
  } else if (title === correctTitle && author !== correctAuthor) {
    feedbackBox.textContent = `📚 You got the title! Now guess the author. (${attemptCount}/${MAX_ATTEMPTS})`;
    feedbackBox.className = "feedback partial";
  } else {
    feedbackBox.textContent = `❌ Nope! Try again. (${attemptCount}/${MAX_ATTEMPTS})`;
    feedbackBox.className = "feedback error";
  }
});
...
document.getElementById('timeHintBtn').addEventListener('click', function() {
  ...
  hintDisplay.textContent = `📅 You read this book between ${startYear} and ${endYear}`;
...
function fuzzyMatch(query, text) {
  ...
  return Math.max(0, 1 - distance / Math.max(cleanQuery.length, cleanText.length));
}
...
function showSuggestions(inputId, listId, key) {
  ...
  const updateSuggestions = () => {
    ...
    list.innerHTML = '';
    selectedIndex = 0;
    if (!query) { list.style.display = 'none'; currentMatches = []; return; }
...
const authorSuggestions = showSuggestions('author', 'authorSuggestions', 'author');
